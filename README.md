...
2. 首次打开客户端，在“概览”的引导中填写本机唯一的“设备标识”、服务器域名或 IP（不要带 `http://`）和管理端口。
3. 输入配对码并点击“配对并填充配置”。连接端口、Token 和本设备独立控制凭据会自动保存；此时配置仍待启动。
4. 检查首条端口映射的本地服务与远程端口，明确确认后点击“保存并启动 frpc”。“frpc 运行中”只表示本机进程状态；若映射不可用，进入“连接设置与日志”查看运行日志，并核查服务器与端口。

已有配置会直接进入映射控制台。旧版服务端仍可在首次引导选择“旧服务器 Token”，已有用户可从“连接设置与日志”手动调整接入端口和 Token。运行中保存映射后需停止并重新启动 frpc，才能应用新配置。

配对码仅 10 分钟有效且只能使用一次，完整配对码不会经网络发送。Token 与设备凭据都不要截图、公开或提交到源码。所有电脑完成配对后，建议在服务端设备中心关闭“旧版远控兼容”，以强制每设备身份和撤销策略。
