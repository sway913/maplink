...
1. 登录 MapLink Server 管理后台，在“设备中心”生成一次性配对码。
2. 打开客户端“连接配置”，填写本机唯一的“设备标识”、服务器域名或 IP（不要带 `http://`）和管理端口。
3. 输入配对码并点击“配对并填充配置”。连接端口、Token 和本设备独立控制凭据会自动保存。
4. 点击“保存并启动”。状态显示原版 frpc 已运行即连接成功。

配对码仅 10 分钟有效且只能使用一次，完整配对码不会经网络发送。旧版服务端仍可手动填写 FRP 接入端口、管理端口和 Token；Token 与设备凭据都不要截图、公开或提交到源码。所有电脑完成配对后，建议在服务端设备中心关闭“旧版远控兼容”，以强制每设备身份和撤销策略。
...
1. 两台电脑分别使用配对码接入同一服务器，并使用不同设备标识。
...
连接前或连接中可以选择 720P/30、1080P/60 或 4K/60。状态栏显示实际 FPS 与实时带宽；网络或被控端编码性能不足时，目标帧率不会通过丢失安全边界来强行保证。点击“全屏窗口”会打开独立原生窗口，顶部工具栏可切换画质、双向文本剪贴板和全屏状态。剪贴板只在内容变化时同步，单次文本最大 64 KiB。

v0.8.1 起控制端复用同一 HTTPS/TLS 连接池，并按显示器刷新周期只呈现最新到达的帧，避免逐帧握手和突发帧排队造成画面周期性停顿。

v0.8.2 起鼠标、键盘输入使用独立低延迟通道，不再等待画面上传完成；全屏窗口输入只做一次 16 ms 合并，连续鼠标移动只保留尚未处理的最新坐标。
