...
1. 在项目的 [GitHub Releases](https://github.com/sway913/maplink/releases) 下载最新版。
2. Windows 推荐下载 `.exe` 安装包；免安装使用可下载 Windows 便携 ZIP，解压后运行 MapLink。
3. Apple Silicon Mac 下载 `.dmg`，打开后把 MapLink 拖入“应用程序”。首次启动若被系统拦截，请在“系统设置 → 隐私与安全性”中确认打开。

### 2. 连接自己的 MapLink 服务器

服务端是独立开源项目：[sway913/maplink-server](https://github.com/sway913/maplink-server)。可从 [MapLink Server Releases](https://github.com/sway913/maplink-server/releases) 下载服务器发行包，部署方法见服务端项目 README。本客户端不会自动提供公共中转服务器，需要连接你自己部署并管理的 MapLink Server。

打开“连接配置”，依次填写：

- **设备标识**：每台电脑使用不同名称，只能包含字母、数字、短横线和下划线，例如 `office-pc`、`home-mac`。
- **服务器 IP**：填写服务端域名或 IP，不要带 `http://`、`https://` 和路径。
- **接入端口**：选择服务端允许的 FRP 接入端口，默认从 `7000` 开始。
- **管理端口**：服务端管理 API 端口，默认是 `7400`。
- **认证 Token**：从自己的 MapLink Server 管理台复制；不要截图、公开或提交到源码。
- **传输协议**：不确定时保持 `TCP`。

点击“保存并启动”。顶部和“原版 frpc 控制”区域显示运行中即表示连接成功；失败时先查看页面底部提示和“最近日志”。

### 3. 添加普通端口映射

在“端口映射”点击“添加映射”，填写本地服务和对外端口。例如把本机 Web 服务开放出去：

| 字段 | 示例 | 说明 |
| --- | --- | --- |
| 名称 | `web-demo` | 同一设备内不可重复 |
| 类型 | `TCP` | 按本地服务需要选择 TCP 或 UDP |
| 本地 IP | `127.0.0.1` | 服务就在本机时使用此值 |
| 本地端口 | `8080` | 本地程序实际监听端口 |
| 远程端口 | `30080` | 必须位于服务端允许范围，且不能和其他映射冲突 |

填写后再次点击“保存并启动”。不要把数据库、远程桌面或管理后台直接暴露到公网，除非已经配置可靠认证和访问限制。

### 4. 使用 SSH 终端

1. 两台设备都进入“远程连接 → SSH 连接”，等待 OpenSSH 检测完成。Windows 缺少组件时点击“安装并启用 OpenSSH”；macOS 按提示开启“远程登录”。
2. 在每台被连接的电脑填写“本机 SSH 用户名”和一个唯一的“本机公网端口”，点击“添加本机 SSH 映射”，然后回到“连接配置”保存并启动 frpc。
3. 在控制端手动填写对方系统、对方 SSH 用户名和对方公网端口，点击“连接终端”。
4. 连接成功后直接在下方黑色终端输入命令；提示符、目录切换、ANSI 颜色和方向键历史都保留在同一会话中。

两台电脑的公网端口不能相同，例如 A 使用 `30022`、B 使用 `30023`。MapLink 使用独立 Ed25519 密钥完成免密连接，私钥只保存在创建它的本机。

### 5. 使用远程桌面

1. 两台电脑连接同一台 MapLink Server，使用相同 Token 和不同设备标识。
2. 被控端进入“远程连接 → 远程控制”，勾选“允许其他 MapLink 设备控制本机”。
3. Windows 接受管理员权限提示；macOS 首次使用时授予“屏幕录制”和“辅助功能”权限。
4. 控制端刷新“在线远程设备”，选择目标后点击“远程连接”。列表为空时，确认对方客户端仍在线并已开启允许控制。

远程画面和输入通过自己的 MapLink Server 中转；服务端不录屏、不落盘，一台被控设备同一时间只接受一个活动会话。

### 6. 检查更新

进入“关于”，点击“检查更新”。发现新版本后，客户端会从 MapLink 官方 GitHub Release 下载适合当前系统的安装文件、校验 SHA-256，并启动 Windows 安装程序或打开 macOS 安装镜像。

## 常见问题

- **SSH 显示 `Connection timed out`**：确认双方 frpc 都在运行、填写的是对方公网端口、服务器防火墙已允许该端口；VPN 或代理也可能改变路由。
- **SSH 显示 `Permission denied`**：核对对方系统用户名，并确认被控端 OpenSSH 已启动且 MapLink 公钥已加入授权文件。
- **Windows 一直显示正在配置 OpenSSH**：Windows 可能正在下载可选组件，保持网络畅通并等待页面阶段提示；完成后点击“重新检测”。
- **远程设备列表为空**：确认两端使用相同服务器、管理端口和 Token，设备标识不同，并在被控端开启允许远程控制。
- **出现 `plugin:event|listen not allowed by ACL`**：这是旧安装包权限不完整，请升级到 `v0.6.4` 或更高版本。
- **远程桌面有画面但不能操作**：Windows 重新以管理员身份启动；macOS 检查“屏幕录制”和“辅助功能”是否都已授权。
