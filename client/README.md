...
- v0.7.0 支持使用服务端一次性配对码自动获取连接配置和每设备独立控制凭据；完整配对码不经网络发送，服务端响应使用认证加密，旧版手动 Token 配置继续兼容；
- v0.7.1 修复远程桌面连接参数名与 Tauri 命令不一致的问题；设备列表改为进入远程控制页时自动刷新一次，后台状态轮询不再重复刷新，仍可随时手动刷新；
- v0.7.2 修复 macOS 权限探测每 5 秒主动触发辅助功能弹窗的问题；应用启动和后台重试改为静默预检，只有用户主动重新开启远控时才请求一次权限，并分别提示未生效的屏幕录制或辅助功能权限；
- v0.8.1 复用远控 HTTPS/TLS 连接池，并按屏幕刷新周期呈现最新帧，消除逐帧重连和突发渲染导致的周期性卡顿；
- v0.8.2 将鼠标和键盘输入从画面上传中拆分为独立低延迟通道，减少远程鼠标跳动和操作延迟；
- v0.8.0 移除远程桌面固定低帧率节流，支持 720P/30、1080P/60、4K/60 三档画质与实际 FPS/带宽显示；增加独立全屏远控窗口及双向文本剪贴板同步；
- v0.7.3 允许 Tauri WebView 显示服务器中转的 `data:` 实时画面帧，修复会话已连接但画面被内容安全策略拦截的问题；读取画面失败时主动关闭远控会话，避免目标设备被旧会话占用；
...
1. 推荐在服务端“设备中心”为两台设备分别生成一次性配对码；客户端配对后会自动填写连接信息并保存独立控制凭据。旧服务器仍可手动填写同一 Token。
2. 在被控设备勾选“允许其他 MapLink 设备控制本机”。Windows 接受 UAC 管理员提示；macOS 在系统设置中允许当前版本 MapLink 的“屏幕录制”和“辅助功能”，授权后完全退出并重新打开 MapLink。临时签名安装包升级后可能需要重新授权当前版本。
